...
updateConfig(){
	...
	if ! grep -Fq "ocpp_enable=" $ConfigFile; then
		echo "ocpp_enable=0" >> $ConfigFile
	fi
	if ! grep -Fq "ocpp_enable_bAuth=" $ConfigFile; then
		echo "ocpp_enable_bAuth=0" >> $ConfigFile
	fi
	if ! grep -Fq "ocpp_endpoint=" $ConfigFile; then
		echo "ocpp_endpoint=0" >> $ConfigFile
	fi
	if ! grep -Fq "ocpp_CPID=" $ConfigFile; then
		echo "ocpp_CPID=0" >> $ConfigFile
	fi
	if ! grep -Fq "ocpp_bAuth_User=" $ConfigFile; then
		echo "ocpp_bAuth_User='User'" >> $ConfigFile
	fi
	if ! grep -Fq "ocpp_bAuth_Password=" $ConfigFile; then
		echo "ocpp_bAuth_Password='password'" >> $ConfigFile
	fi
	if ! grep -Fq "discovergypass=" $ConfigFile; then
		echo "discovergypass=''" >> $ConfigFile
	else
